...
public class ReturnItem extends AppCompatActivity {
    ...
    public void onBackPressed() {
        super.onBackPressed();
        startActivity(new Intent(ReturnItem.this,OrdersPage.class));
        finish();
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        order_ref2= FirebaseDatabase.getInstance().getReference("ORDERS_STATUS").child(user.getUid());   mAuth = FirebaseAuth.getInstance();
        user = mAuth.getCurrentUser();
        details=findViewById(R.id.details);
        backpress=findViewById(R.id.backpress);
        backpress.setOnClickListener(new View.OnClickListener() {
        ...
        SubmitBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (selectedId == 0)
                    Toast.makeText(ReturnItem.this, "Select reason", Toast.LENGTH_SHORT).show();
                else {
                    ...
                    fstatus.setStatus("Return");
                    fstatus.setLocation("NULL");
                    order_ref.child("STATUS")
                            .child(getIntent().getStringExtra("ORDERKEY").toString())
                            .setValue(fstatus);
                    Toast.makeText(ReturnItem.this, "Order Cancelled", Toast.LENGTH_SHORT).show();

                    order_ref2.child(getIntent().getStringExtra("ORDERKEY").toString()).setValue("Return");
                    Intent intent=new Intent(ReturnItem.this,returnSuccess.class);
                    intent.putExtra("PIC","RETURN");
